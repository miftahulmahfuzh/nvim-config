...
-- No more hardcoded paths. We use vim.fn.executable to find nvim.
-- If you need to override this, you can do it here in one place.
M.config = {
  nvim_executable = vim.fn.executable("nvim") or "/usr/bin/nvim",
}
...
local function send_text(selected_text)
  ...
  if file_path == "" then
    vim.notify("Cannot get context from an unsaved buffer. Save the file.", vim.log.levels.WARN, { title = "OpenCode" })
    return
  end
  ...
  if target_pane_id then
    ...
    vim.fn.system(string.format("tmux send-keys -t %s Escape", target_pane_id))
    vim.notify(
      "Sent selection to existing OpenCode pane.",
      vim.log.levels.INFO,
      { title = "OpenCode" }
    )
  else
