...
local function send_files_to_opencode(selected_files)
  ...
  if target_pane_id then
    ...
    vim.notify(
      "Sent " .. #selected_files .. " additional file(s) to existing OpenCode pane.",
      vim.log.levels.INFO,
      { title = "OpenCode" }
    )
  else
...
local function send_text_to_opencode(selected_text)
  ...
  if file_path == "" then
    vim.notify(
      "Cannot get context from an unsaved buffer. Please save the file first.",
      vim.log.levels.WARN,
      { title = "OpenCode" }
    )
    return
  ...
  if target_pane_id then
    ...
    vim.notify(
      "Sent selection from " .. vim.fn.expand("%:t") .. " to existing OpenCode pane.",
      vim.log.levels.INFO,
      { title = "OpenCode" }
    )
  else
    ...
    vim.fn.system(tmux_cmd)
    vim.notify(
      "Sent selection from " .. vim.fn.expand("%:t") .. " to NEW OpenCode pane.",
      vim.log.levels.INFO,
      { title = "OpenCode" }
    )
  end
