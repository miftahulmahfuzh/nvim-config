...
-- Enhanced vim-flog Git log mappings
-- Add these to your existing mappings.lua file

-- Basic Flog commands
vim.keymap.set("n", "<leader>gf", ":Flog -all<CR>", { desc = "Git: Flog all branches" })
vim.keymap.set("n", "<leader>gF", ":Flog<CR>", { desc = "Git: Flog current branch" })
vim.keymap.set("n", "<leader>gL", ":Flog -path=% -all<CR>", { desc = "Git: Flog current file (all branches)" })

-- Focused views
vim.keymap.set("n", "<leader>gr", ":Flog -reflog<CR>", { desc = "Git: Flog reflog" })

-- Date-based logs
vim.keymap.set("n", "<leader>gD", function()
	local days = vim.fn.input("Show commits from last N days (default 7): ")
	if days == "" then
		days = "7"
	end
	vim.cmd("Flog -since=" .. days .. ".days.ago")
end, { desc = "Git: Flog recent commits" })

-- Search in commit messages
vim.keymap.set("n", "<leader>gs", function()
	local search = vim.fn.input("Search in commit messages: ")
	if search ~= "" then
		vim.cmd("Flog -grep=" .. vim.fn.shellescape(search))
	end
end, { desc = "Git: Flog search commits" })

-- Branch comparison
vim.keymap.set("n", "<leader>gb", function()
	local branch = vim.fn.input("Compare with branch (default: main): ")
	if branch == "" then
		branch = "main"
	end
	local current_branch = vim.fn.system("git branch --show-current"):gsub("\n", "")
	vim.cmd("Flog " .. branch .. ".." .. current_branch)
end, { desc = "Git: Flog branch comparison" })

-- Reverse comparison (what's in target branch that's not in current)
vim.keymap.set("n", "<leader>gB", function()
	local branch = vim.fn.input("What's in branch that's not in current (default: main): ")
	if branch == "" then
		branch = "main"
	end
	local current_branch = vim.fn.system("git branch --show-current"):gsub("\n", "")
	vim.cmd("Flog " .. current_branch .. ".." .. branch)
end, { desc = "Git: Flog reverse comparison" })

-- Show commits that modified specific lines in current file
vim.keymap.set("v", "<leader>gl", function()
	-- Get selected line range
	local start_line = vim.fn.line("'<")
	local end_line = vim.fn.line("'>")
	local file = vim.fn.expand("%")
	vim.cmd("Flog -L" .. start_line .. "," .. end_line .. ":" .. file)
end, { desc = "Git: Flog line history (visual selection)" })

-- Show commits in a specific directory
vim.keymap.set("n", "<leader>gd", function()
	local dir = vim.fn.input("Directory path (relative to repo root): ", "", "file")
	if dir ~= "" then
		vim.cmd("Flog -path=" .. vim.fn.shellescape(dir))
	end
end, { desc = "Git: Flog directory history" })

-- Combined filters example
vim.keymap.set("n", "<leader>gx", function()
	local days = vim.fn.input("Days ago (default 30): ")
	if days == "" then
		days = "30"
	end
	local author = vim.fn.input("Author (leave empty for any): ")

	local cmd = "Flog --since=" .. days .. ".days.ago --no-merges"
	if author ~= "" then
		cmd = cmd .. " --author=" .. vim.fn.shellescape(author)
	end
	vim.cmd(cmd)
end, { desc = "Git: Flog custom filter" })
